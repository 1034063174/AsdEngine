...
struct logger_msg {
  static constexpr uint32_t max_msg_length = 512;
...
enum hypercall_code : uint64_t {
  ...
  hypercall_hide_virtual_page_range,
  hypercall_create_ept_hook,
  hypercall_set_rflage,
  hypercall_install_remote_call,
  hypercall_remote_excute_finish
};
...
void* vmx_hv_begin(void* a = nullptr, void* b = nullptr);
inline void vmx_hv_end(void* exit);

void asm_vmx_excute_finish();
...
//hide physicals for linear address range
inline bool hide_linear_range(uint64_t const cr3, uint64_t const vir_addr, size_t const lenth, uint64_t const phy_addr) {
    hv::hypercall_input input;
    input.code = hv::hypercall_hide_virtual_page_range;
    input.key = hv::hypercall_key;
    input.args[0] = cr3;
    input.args[1] = vir_addr;
    input.args[2] = lenth;
    input.args[3] = phy_addr;
    return hv::vmx_vmcall(input);
}
...
inline void* create_ept_hook(uint64_t const cr3, uint64_t const target, uint64_t const detour, uint64_t const fake) {

    hv::hypercall_input input;
    input.code = hv::hypercall_create_ept_hook;
    input.key = hv::hypercall_key;
    ...
    return (void*)hv::vmx_vmcall(input);
}

inline void ept_excute_finish(uint32_t* pStatus) {
    //asm_vmx_excute_finish();
    hv::hypercall_input input;
    input.code = hv::hypercall_remote_excute_finish;
    input.key = hv::hypercall_key;

    input.args[0] = (uint64_t)pStatus;

    hv::vmx_vmcall(input);
}
//void create_ept_excute(vcpu_ept_data & ept, cr3 const cr3, uint64_t const tar_phy, uint64_t const exc_addr, uint32_t* const pStatus)
inline void install_ept_remote_excute(uint64_t cr3, uint64_t tar_phy, uint64_t exc_addr, uint32_t* pStatus) {

    hv::hypercall_input input;
    input.code = hv::hypercall_install_remote_call;
    input.key = hv::hypercall_key;

    input.args[0] = cr3;
    input.args[1] = tar_phy;
    input.args[2] = exc_addr;
    input.args[3] = (uint64_t)pStatus;
